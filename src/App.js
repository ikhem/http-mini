...
class App extends Component {
  ...
  recruitTroop(event) {
    ...
    // Check to see if there is a new recruit 
    // if there is, call addTroop and pass in the new recruit 
    // When we get the recurit back, call Troop()
    ...
    const enemies = this.state.enemies.map((enemy, i) => (
      <ul className="army">
        ...
        <ul className="minions">
          {enemy.minions.map((minion, i) => (
            <li onClick={
              () => {
                this.transformMinion(enemy.shortname, minion.id);
